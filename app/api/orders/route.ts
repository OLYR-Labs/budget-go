...
export async function POST(request: Request) {
  try {
    ...
    if (!branchId?.trim()) {
      return NextResponse.json({ error: "Branch information is required." }, { status: 400 });
    }
    ...
    if (paymentMethod !== "cod") {
      return NextResponse.json({ error: "Only Cash on Delivery is currently available." }, { status: 400 });
    }
    ...
    const normalizedItems: OrderItemInput[] = Array.from(quantityMap.entries()).map(
    ...
    // Never trust distance or fee values from the browser. Recalculate from the branch and map point.
    ...
    if (deliveryDistanceKm > branch.deliveryRadius) {
      return NextResponse.json(
        {
          error: `This location is outside the ${branch.deliveryRadius} km delivery radius of ${branch.name}.`,
        },
        { status: 422 },
      );
    }
    ...
    for (const cartItem of normalizedItems) {
      ...
      if (!stockItem) {
        return NextResponse.json(
          { error: "One of the products in your cart is no longer available at this branch." },
          { status: 409 },
        );
      }
    ...
    const order = await prisma.$transaction(async (tx) => {
      for (const cartItem of normalizedItems) {
        ...
        if (stockItem.stock < cartItem.quantity) {
          throw new Error(`INSUFFICIENT_STOCK:${cartItem.productId}:${stockItem.stock}`);
        }
      }
    ...
    if (error instanceof Error) {
      if (error.message.startsWith("PRODUCT_UNAVAILABLE:")) {
        return NextResponse.json({ error: "One of the products in your cart is no longer available." }, { status: 409 });
      }

      if (error.message.startsWith("INSUFFICIENT_STOCK:")) {
        const stock = error.message.split(":")[2];
        return NextResponse.json(
          { error: `The requested quantity is no longer available. Only ${stock} item${stock === "1" ? "" : "s"} remain in stock.` },
          { status: 409 },
        );
      }

      if (error.message.startsWith("STOCK_CHANGED:")) {
        return NextResponse.json({ error: "Stock changed while placing your order. Please refresh your cart and try again." }, { status: 409 });
      }
    }
