...
export default function CustomerRegisterPage() {
  const [name, setName] = useState(""); const [phone, setPhone] = useState(""); const [email, setEmail] = useState(""); const [address, setAddress] = useState(""); const [city, setCity] = useState("");
  const [password, setPassword] = useState(""); const [confirmPassword, setConfirmPassword] = useState(""); const [showPassword, setShowPassword] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState("");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    } catch (registerError) { console.error("Customer registration failed:", registerError); setError("Something went wrong while creating your account. Please try again."); }
      ...
      <div className="relative grid w-full max-w-6xl overflow-hidden rounded-[2rem] border border-border/70 bg-card shadow-2xl shadow-black/[0.08] lg:grid-cols-[0.82fr_1.18fr]">
        ...
        <section className="p-6 sm:p-9 lg:p-10 xl:p-12">
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2"><Field icon={<User className="h-4 w-4" />} label="Full name"><input required autoComplete="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your full name" className="field-input" /></Field><Field icon={<Phone className="h-4 w-4" />} label="Phone number"><input required type="tel" autoComplete="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="07X XXX XXXX" className="field-input" /></Field></div>
            ...
            <div className="grid gap-4 sm:grid-cols-[1.35fr_0.65fr]"><Field icon={<Home className="h-4 w-4" />} label="Delivery address"><textarea required autoComplete="street-address" value={address} onChange={(e) => setAddress(e.target.value)} rows={2} placeholder="House number, street, landmark" className="field-input h-auto py-3" /></Field><Field icon={<MapPin className="h-4 w-4" />} label="City / area"><input required autoComplete="address-level2" value={city} onChange={(e) => setCity(e.target.value)} placeholder="City or area" className="field-input" /></Field></div>
            <div className="grid gap-4 sm:grid-cols-2"><Field icon={<LockKeyhole className="h-4 w-4" />} label="Password"><PasswordInput value={password} onChange={setPassword} show={showPassword} onToggle={() => setShowPassword((v) => !v)} placeholder="At least 8 characters" /></Field><Field icon={<LockKeyhole className="h-4 w-4" />} label="Confirm password"><input required minLength={8} type={showPassword ? "text" : "password"} autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Re-enter password" className="field-input" /></Field></div>
            <p className="flex items-center gap-2 text-[10px] text-muted-foreground"><CheckCircle2 className="h-3.5 w-3.5 text-accent" />Your address will be saved as your default delivery address.</p>
