...
export default function Home() {
  ...
  const loadProducts = useCallback(async (branchId: string) => {
    ...
    try {
      ...
      if (cached) {
        ...
        if (Array.isArray(parsed)) { setProducts(parsed); setLoading(false); usedCachedProducts = true; }
      }
    } catch (cacheError) { console.warn("Failed to read cached homepage products:", cacheError); }
    try {
  ...
  useEffect(() => {
    ...
    const restore = () => {
      ...
      if (target === null) { holdingSavedPositionRef.current = false; hasRestoredScrollRef.current = true; return; }
      ...
      requestAnimationFrame(() => {
        ...
        if (reached) { hasRestoredScrollRef.current = true; holdingSavedPositionRef.current = false; }
      });
    };
  ...
  useEffect(() => {
    ...
    const savedPath = window.sessionStorage.getItem(HOME_SCROLL_PATH_KEY);
    const saved = Number(window.sessionStorage.getItem(HOME_SCROLL_KEY));
    if (savedPath !== path || !Number.isFinite(saved) || saved <= 0) { holdingSavedPositionRef.current = false; return; }
    const restore = () => {
      if (hasRestoredScrollRef.current) return;
      const maxScroll = Math.max(0, document.documentElement.scrollHeight - window.innerHeight);
      ...
      requestAnimationFrame(() => {
        restoringScrollRef.current = false;
        if (Math.abs(window.scrollY - saved) <= 4) { hasRestoredScrollRef.current = true; holdingSavedPositionRef.current = false; }
      });
    };
