...
export default function OrderAssignmentTable({
  ...
  const [selected, setSelected] = useState<Record<string, string>>(
    Object.fromEntries(orders.map((order) => [order.id, order.deliveryStaff?.id ?? ""])),
  );
        ...
        <table className="w-full min-w-[1200px] text-left">
          <thead className="border-b border-border bg-muted/30">
            <tr className="text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">
              ...
              {canAssign && <th className="px-5 py-4">Delivery</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {orders.length === 0 ? (
              <tr><td colSpan={canAssign ? 8 : 7} className="px-5 py-12 text-center text-sm text-muted-foreground">No orders found.</td></tr>
            ) : orders.map((order) => {
              ...
              return (
                <tr key={order.id} className="hover:bg-muted/20">
                  <td className="px-5 py-4">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">{new Intl.DateTimeFormat("en-LK", { dateStyle: "medium", timeStyle: "short" }).format(new Date(order.createdAt))}</p>
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <span className={`rounded-full px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider ${statusClass(order.status)}`}>{order.status.replaceAll("_", " ")}</span>
                    {order.deliveryStaff && <p className="mt-2 text-[10px] text-muted-foreground">Driver: {order.deliveryStaff.user.name}</p>}
                  </td>
                  {canAssign && (
                    <td className="px-5 py-4">
                      <div className="flex min-w-[250px] gap-2">
                        <select
                          ...
                          onChange={(event) => setSelected((current) => ({ ...current, [order.id]: event.target.value }))}
                          disabled={busy === order.id || order.status === "DELIVERED" || order.status === "CANCELLED"}
                          className="h-10 flex-1 rounded-xl border border-border bg-background px-3 text-xs outline-none focus:border-accent/50 disabled:opacity-50"
                        >
                          <option value="">{branchStaff.length ? "Unassigned" : "No drivers"}</option>
                          {branchStaff.map((staff) => <option key={staff.id} value={staff.id}>{staff.user.name}{staff.user.phone ? ` · ${staff.user.phone}` : ""}</option>)}
                        </select>
                        <button
                          type="button"
                          onClick={() => assign(order.id)}
                          disabled={busy === order.id || !branchStaff.length || order.status === "DELIVERED" || order.status === "CANCELLED"}
                          className="h-10 rounded-xl bg-primary px-3 text-xs font-bold text-primary-foreground disabled:opacity-40"
                        >
                          {busy === order.id ? "Saving" : isPending ? "Assign" : "Save"}
                        </button>
                      </div>
                    </td>
                  )}
