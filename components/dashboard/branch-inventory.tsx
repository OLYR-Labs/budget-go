  ...
  async function addProduct(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/dashboard/branch/products/create", {
        ...
        body: JSON.stringify({ name, category, barcode, price, stock, imageUrl: imageUrl || null }),
      });
      const data = await response.json();
    ...
    <section className="min-h-screen bg-background px-5 py-8 text-foreground sm:px-8 lg:px-10">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-5 flex flex-col gap-3 sm:flex-row">
          ...
          <button type="button" onClick={() => { setError(""); setShowAdd((value) => !value); }} className="h-11 rounded-xl bg-primary px-4 text-sm font-bold text-primary-foreground">+ Add product</button>
        </div>

        {showAdd && (
          <form onSubmit={addProduct} className="mb-5 grid gap-4 rounded-2xl border border-border bg-card p-5 sm:grid-cols-2 lg:grid-cols-6">
            <div className="lg:col-span-2">
              <label className="text-xs font-semibold">Product name
            ...
            <label className="text-xs font-semibold">Category
              <input required value={category} onChange={(event) => setCategory(event.target.value)} placeholder="Beverages" className="mt-2 h-11 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none focus:border-accent/50" />
            </label>
            <label className="text-xs font-semibold">Product code / barcode
            ...
            <div className="sm:col-span-2 lg:col-span-6">
              <label className="text-xs font-semibold">Product photo
                <div className="mt-2 flex flex-wrap items-center gap-3">
                  <label className="inline-flex h-11 cursor-pointer items-center rounded-xl border border-border bg-background px-4 text-sm font-semibold hover:bg-muted">
                    {photoName ? "Change photo" : "Choose photo"}
                    <input type="file" accept="image/jpeg,image/png,image/webp" onChange={handlePhoto} className="sr-only" />
                  </label>
                  {photoName && <span className="text-xs text-muted-foreground">{photoName}</span>}
                  {imageUrl && <img src={imageUrl} alt="Product preview" className="h-12 w-12 rounded-lg border border-border object-cover" />}
                </div>
              </label>
            </div>
            <div className="flex flex-wrap items-center gap-2 sm:col-span-2 lg:col-span-6">
              <button disabled={busy === "add"} className="h-11 rounded-xl bg-primary px-5 text-sm font-bold text-primary-foreground disabled:opacity-50">{busy === "add" ? "Adding..." : "Add product"}</button>
              <button type="button" onClick={resetForm} className="h-11 rounded-xl border border-border px-5 text-sm font-semibold">Cancel</button>
              <p className="text-xs text-muted-foreground">This creates the product in the catalog and adds its stock to {branchName}.</p>
            </div>
          </form>
        )}
              ...
              <tbody className="divide-y divide-border">
                {loading ? <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-muted-foreground">Loading inventory...</td></tr> : items.length === 0 ? <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-muted-foreground">No inventory items found.</td></tr> : items.map((item) => (
                  <tr key={item.id} className="align-middle">
                    <td className="px-5 py-4"><div className="flex items-center gap-3">{item.product.imageUrl ? <img src={item.product.imageUrl} alt="" className="h-10 w-10 rounded-lg border border-border object-cover" /> : <div className="h-10 w-10 rounded-lg border border-border bg-muted" />}<div><div className="font-semibold">{item.product.name}</div>{item.product.category && <div className="mt-1 text-xs text-muted-foreground">{item.product.category.name}</div>}</div></div></td>
                    <td className="px-5 py-4 text-sm text-muted-foreground">{item.product.barcode ?? item.product.sku}</td>
        ...
        <p className="mt-4 text-xs text-muted-foreground">Branch staff can view inventory, add new catalog products, increase/decrease stock, and remove products. Orders, revenue, and delivery assignment remain unavailable.</p>
      </div>
    </section>
  );
}
