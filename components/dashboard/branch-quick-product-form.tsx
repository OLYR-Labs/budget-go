  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/dashboard/branch/products/create", {
        ...
        body: JSON.stringify({ name, category, barcode, price, stock }),
      });
  ...
  return (
    <div className="mb-5 rounded-2xl border border-accent/20 bg-accent/5 p-4">
      <button type="button" onClick={() => setOpen((value) => !value)} className="text-sm font-black text-foreground">
        {open ? "− Close product entry" : "+ Add a new product"}
      </button>

      {open && (
        <form onSubmit={submit} className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <label className="text-xs font-semibold sm:col-span-2 lg:col-span-1">
            Product name
          ...
          <div className="flex flex-wrap items-center gap-3 sm:col-span-2 lg:col-span-5">
            <button disabled={busy} className="h-11 rounded-xl bg-primary px-5 text-sm font-bold text-primary-foreground disabled:opacity-50">{busy ? "Adding..." : "Add product"}</button>
            <p className="text-xs text-muted-foreground">The product is added to this branch with the category you enter.</p>
          </div>
          {message && <p className="text-sm font-semibold text-emerald-600 sm:col-span-2 lg:col-span-5">{message}</p>}
          {error && <p className="text-sm font-semibold text-destructive sm:col-span-2 lg:col-span-5">{error}</p>}
        </form>
      )}
    </div>
  );
